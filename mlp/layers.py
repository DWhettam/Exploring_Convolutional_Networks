...
class ConvolutionalLayer(LayerWithParameters):
    ...
    def fprop(self, inputs):
        ...
        stride = 1
        padding = 0  
        h_out = int((inputs.shape[2] - self.kernels.shape[2] + 2 * padding) / stride + 1)
        w_out = int((inputs.shape[3] - self.kernels.shape[3] + 2 * padding) / stride + 1)        
        out = np.zeros([inputs.shape[0], self.kernels.shape[0], h_out, w_out])
        for batch_item in range(inputs.shape[0]):
            for output_channel in range(self.num_output_channels):  
                for input_channel in range(self.num_input_channels):                                                               
                    I = inputs[batch_item, input_channel, :,:]
                    K = self.kernels[output_channel, input_channel, :, :]                    
                    output = scipy.signal.correlate2d(I,K, 'valid') 
                    out[batch_item, output_channel, :, :] += output  
                out[batch_item, output_channel, :, :] += self.biases[output_channel, np.newaxis]
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        Returns:
            ...
            (batch_size, num_input_channels, input_height, input_width).
        """      
        grad_wrt_inputs = np.zeros(inputs.shape)
        
        for batch_item in range(inputs.shape[0]):
            for output_channel in range(self.num_output_channels):                
                for input_channel in range(self.num_input_channels):                     
                    E = grads_wrt_outputs[batch_item, output_channel, :,:]
                    K = self.kernels[output_channel, input_channel, :, :]
                    output = scipy.signal.correlate2d(E,np.flip(K))                    
                    grad_wrt_inputs[batch_item, input_channel, :, :] += output                                                
    ...
    def grads_wrt_params(self, inputs, grads_wrt_outputs):
        ...
        for i in range(self.num_output_channels):
            grads_wrt_biases.append(np.sum(grads_wrt_outputs[:, i, :, :]))                               
        grads_wrt_biases = np.array(grads_wrt_biases)
        
        
        grad_wrt_kernels = np.zeros(self.kernels_shape)
                                                 
        for input_channel in range(self.num_input_channels):                             
            for batch_item in range(inputs.shape[0]):  
                for output_channel in range(self.num_output_channels):                                                
                        I = inputs[batch_item, input_channel, :, :]
                        G = np.transpose(grads_wrt_outputs, [1,0,2,3])[output_channel, batch_item, :,:]
                        output = scipy.signal.correlate2d(I, G, 'valid')                        
                        grad_wrt_kernels[output_channel, input_channel, :, :] += output
        
        if self.kernels_penalty is not None:
            grad_wrt_kernels += self.kernels_penalty.grad(parameter=self.kernels)

        if self.biases_penalty is not None:
            grads_wrt_biases += self.biases_penalty.grad(parameter=self.biases)
        
        return grad_wrt_kernels, grads_wrt_biases
...
class MaxPooling2DLayer(Layer):
    ...
    def fprop(self, inputs):
        ...
        """         
        out_h = int((self.input_height - self.size)/self.stride + 1)
        out_w = int((self.input_width - self.size)/self.stride + 1)       
        
        out = np.zeros([inputs.shape[0], inputs.shape[1], out_h, out_w])
            ...
            for channel in range(inputs.shape[1]):
                for h in range(out_h):
                    for w in range(out_w):
                        h1 = h * self.stride
                        h2 = h * self.stride + self.size
                        w1 = w * self.stride
                        w2 = w * self.stride + self.size
                        
                        kernel = inputs[batch_item, channel, h1:h2, w1:w2]
                        out[batch_item, channel, h, w] = np.max(kernel)                                                
        return out
            ...
            for channel in range(inputs.shape[1]):
                for h in range(out_h):
                    for w in range(out_w):
                        ...
                        I = inputs[batch_item, channel, h1:h2, w1:w2]
                        G = grads_wrt_outputs[batch_item, channel, h, w]
                                                
                        max_ind = np.argmax(I)
                        temp = np.zeros(np.ndarray.flatten(I).shape)
                        temp[max_ind] = 1
                        new_I = temp.reshape(I.shape)                        
                        out[batch_item, channel, h1:h2, w1:w2] = new_I * G 
        return out
